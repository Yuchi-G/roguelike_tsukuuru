...
type PendingDialogAction = "save-project" | "save-project-as";
...
export class ConfigPanel {
  ...
  private pendingDialogAction: PendingDialogAction | null = null;
  ...
  private render(): void {
    this.root.innerHTML = [
      ...
      this.renderDatabasePage("enemies", [this.renderEnemySection()]),
      this.renderDatabasePage("items", [this.renderItemSection()]),
      this.renderDatabasePage("skills", [this.renderPlayerSection(), this.renderProgressionSection(), this.renderRenderSection()]),
      this.renderDatabasePage("terrain", [this.renderDungeonSection(), this.renderFloorSection(), this.renderTileSection()]),
      this.renderDatabasePage("events", [this.renderMessageSection(), this.renderProjectSection()]),
      "</div>",
      "</div>",
  ...
  private renderDatabaseTabs(): string {
    ...
    return [
      ...
      ...tabs.map(([tabId, label]) => (
        `<button type="button" role="tab" data-database-tab="${tabId}" class="${this.activeDatabaseTab === tabId ? "is-active" : ""}">${label}</button>`
      )),
      "</div>",
    ].join("");
  }
  ...
  private renderDatabaseList(): string {
    const rows = this.databaseListRows();
    return [
      '<aside class="database-list" aria-label="一覧">',
      `<strong>${escapeHtml(this.databaseTabLabel(this.activeDatabaseTab))}</strong>`,
      '<ol>',
      ...rows.map((row, index) => `<li class="${index === 0 ? "is-selected" : ""}">${escapeHtml(row)}</li>`),
      "</ol>",
      "</aside>",
    ].join("");
  }

  private databaseListRows(): string[] {
    switch (this.activeDatabaseTab) {
      case "enemies":
        return this.config.enemies.map((enemy) => enemy.name);
      case "items":
        return this.config.items.map((item) => item.name);
      case "skills":
        return ["プレイヤー", "成長 / 視界", "Canvas描画"];
      case "terrain":
        return ["ダンジョン", "階層ルール", ...Object.keys(this.config.tiles)];
      case "events":
        return ["ログ文言", "プロジェクト"];
    }
  }
  ...
  private renderDatabasePage(tabId: DatabaseTab, sections: string[]): string {
    return `<section class="database-page${this.activeDatabaseTab === tabId ? " is-active" : ""}" data-database-page="${tabId}">${sections.join("")}</section>`;
  }

  private renderDialog(): string {
    if (!this.pendingDialogAction) return "";

    return [
      '<div class="dialog-backdrop">',
      '<div class="dialog-window" role="dialog" aria-modal="true" aria-label="保存確認">',
      '<p>保存しますか？</p>',
      '<div class="dialog-command-list">',
      '<button type="button" data-dialog-answer="yes">&gt; はい</button>',
      '<button type="button" data-dialog-answer="no">　いいえ</button>',
      "</div>",
      "</div>",
      "</div>",
    ].join("");
  ...
  private handleClick(event: MouseEvent): void {
    ...
    const tabButton = clickedElement.closest<HTMLButtonElement>("button[data-database-tab]");
    if (tabButton) {
      this.applyCurrentForm(tabButton);
      this.activeDatabaseTab = (tabButton.dataset.databaseTab as DatabaseTab | undefined) ?? this.activeDatabaseTab;
      this.render();
      return;
    }

    const dialogButton = clickedElement.closest<HTMLButtonElement>("button[data-dialog-answer]");
    if (dialogButton) {
      const action = this.pendingDialogAction;
      this.pendingDialogAction = null;
      if (dialogButton.dataset.dialogAnswer !== "yes" || !action) {
        this.render();
        return;
      }

      this.applyCurrentForm(dialogButton);
    ...
    if (clickedElement.dataset.action === "save-project") {
      ...
      if (configForm instanceof HTMLFormElement) {
        ...
        this.pendingDialogAction = "save-project";
        this.render();
      }
      return;
    }
    ...
    if (clickedElement.dataset.action === "save-project-as") {
      ...
      if (configForm instanceof HTMLFormElement) {
        this.applyFormConfig(new FormData(configForm));
        this.pendingDialogAction = "save-project-as";
        this.render();
      }
    }
  }
  ...
  private async duplicateTile(tileType: string): Promise<void> {
    const sourceTile = this.config.tiles[tileType];
    if (!sourceTile) return;

    const duplicatedTileType = this.uniqueId(`${tileType}_copy`, (candidateId) => Boolean(this.config.tiles[candidateId]));
    this.config.tiles[duplicatedTileType] = {
      ...structuredClone(sourceTile),
      type: duplicatedTileType,
  ...
  private updateSaveStatusElement(): void {
    const saveStatusElement = document.querySelector<HTMLElement>("#status-save");
    if (saveStatusElement) {
      saveStatusElement.textContent = this.projectInfo.isDirty ? "未保存" : "保存済み";
    }
  }
