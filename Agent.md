...
## その他
- 次のタスクを見つけたときは`./docs/tasks`にtask*.mdとして出力する。*では0001〜の採番を行い、ファイル更新は行わず新規ファイルを作って運用すること。

## 実装後の必須アクション
実装が完了したら、**最後に必ず**以下の形式で「実装意図チェックリスト」を出力すること。テスト・ビルド成功の報告だけで終わらせない。

**目的:** 修正箇所の設計判断や動作変化をユーザーが正しく理解できているか確認するため。

**形式:**
```
## 実装意図チェックリスト
- [ ] （なぜこの設計にしたか、なぜこの分岐が必要か、など設計判断を問う項目）
- [ ] （変更前と変更後で動作がどう変わるかを問う項目）
- [ ] ...
```

**注意:**
- コードの表面的な差分（「○○を追加した」）ではなく、設計判断・動作変化に焦点を当てる
- 変更ファイルが複数ある場合はファイルごとではなく論点ごとにまとめる
- チェックリストを出す前にタスク完了と報告しない